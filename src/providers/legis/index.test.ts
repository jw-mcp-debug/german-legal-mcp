...
describe('LegisProvider', () => {
  it('dispatches search and formats results', async () => {
    const res = await provider().handleToolCall('legis:search', { query: 'q', state: 'XX' });
  ...
  it('renders a document and saves it to a file', async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'legis-')); dirs.push(dir);
    const file = join(dir, 'g.md');
    const saved = await provider().handleToolCall('legis:get', { id: 'g1', state: 'XX', save_path: file });
  ...
  it('extracts a TOC from document headings with depth and range filters', async () => {
    const all = await provider().handleToolCall('legis:toc', { id: 'g1', state: 'XX' });
    expect(all.content[0].text).toContain('§ 1 Zweck');
    expect(all.content[0].text).toContain('Art. 3 Sonderfall');

    const shallow = await provider().handleToolCall('legis:toc', { id: 'g1', state: 'XX', depth: 1 });
    expect(shallow.content[0].text).not.toContain('Art. 3'); // depth 2 filtered out

    const ranged = await provider().handleToolCall('legis:toc', { id: 'g1', state: 'XX', from: '§ 1', to: '§ 2' });
  ...
  it('lists supported jurisdictions', async () => {
    const res = await provider().handleToolCall('legis:states', {});
  ...
  it('errors on an unknown tool and an unsupported state', async () => {
    const p = provider();
    expect((await p.handleToolCall('legis:bogus', {})).isError).toBe(true);
    await expect(p.handleToolCall('legis:search', { query: 'q', state: 'ZZ' }))
