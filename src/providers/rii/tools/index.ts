...
export const riiTools: ToolDefinition[] = [
  {
    ...
    inputSchema: z.object({
      ...
      limit: z.number().optional().default(10).describe('Maximum number of results (default: 10). With source "ALL" the slots are shared across the portals that matched, so no single portal can fill the page on its own.'),
      ...
      page: z.number().int().min(1).optional().default(1).describe('1-based page. Each portal is asked for its own page N, so paging a consolidated search goes deeper into every portal at once. BUND, HB and SN cannot page — the summary names any portal that could not reach the requested page rather than silently repeating page 1.'),
